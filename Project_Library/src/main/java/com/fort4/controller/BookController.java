...
public class BookController extends BaseController {
    ...
    @ResponseBody
    public Map<String, Object> rentBookAjax(@PathVariable int bookId, HttpSession session) {
        ...
        if (rentalMapper.countNotReturned(bookId) > 0) {
            result.put("status", "error");
            result.put("message", "이미 대여중인 도서입니다.");
            return result;
        }
    ...
    @ResponseBody
    public Map<String, Object> returnBookAjax(@PathVariable int bookId, HttpSession session) {
        Map<String, Object> result = new HashMap<>();
        MemberDTO user = getLoginUser(session);
        if (user == null) {
            result.put("status", "error");
        ...
        if (!isAdmin(user) && !rental.getUsername().equals(user.getUsername())) {
            result.put("status", "error");
            result.put("message", "반납 권한이 없습니다.");
            return result;
        }
